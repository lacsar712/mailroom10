...
import (
	...
	"example.com/mailroom/internal/util/filedump"
	"example.com/mailroom/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "Mailroom IMb", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "mailroom"},
	}
